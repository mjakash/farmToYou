...
public class OrderController {
    ...
    public ResponseEntity<OrderResponse> dispatchOrder(@PathVariable Long orderId, @RequestBody DispatchRequest dispatchRequest) {
        OrderResponse response = orderService.dispatchOrder(orderId, dispatchRequest);
        return ResponseEntity.ok(response);
    }
    
    @PostMapping("/{orderId}/pack")
    public ResponseEntity<OrderResponse> packageOrder(@PathVariable Long orderId, @RequestBody PackageOrderRequest packageRequest) {
